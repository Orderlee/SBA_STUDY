...
'''
예외처리 : 예외가 사전에 발생하지 않도록 막음 조치를 취하는 것
IndexError 0으로 나누고자 했을 때
IndexError 인덱스의 범위를 초과하여 접근시도 시 발생 
KeyError 사전에 해당 키가 존재하지 않을 때

except 구문은 1번이상 사용가능하다.

예외처리 방법
try:
    일반적인 코드 작성
except 예외클래스 이름 [as 예외 별칭]:
    적당한 오류 메시지 보여주기 
else:
    예외가 없을 때 하고자 하는 내용 기록
finally:
    예외 발생 여부와 상관 없이 하고자 하는 내용 기록
    주로 마감 작업(파일닫기, 데이터베이스 접속을 종료)
'''