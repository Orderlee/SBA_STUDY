# 참조파일 :tojitext.txt, stopword.txt, alice_color.png
# 토지 파일을 읽어 들여서 워드 클라우드와 막대 그래프 그리기
import numpy as np
from PIL import Image #pillow 설치
from wordcloud import WordCloud # 참고 : pytagclouod
...
class Visiualization:
    ...
    def makeWorCloud(self): # 워드클라우드
        alice_color_file = '../attach/alice_color.png'
        # 이미지를 넘파이 배열로 바꿈
        alice_coloring = np.array(Image.open(alice_color_file))

        fontpath='/Library/Fonts/AdobeGothicStd-Bold.otf'
    ...
    def makeBarChart(self): # 막대그래프
        # result를 이용하여 막대 그래프를 그려보세
...
# 불용어(stopword) :  빈도수에 상관없이 분석에서 배제할 단어들
stop_word_file = '../attach/stopword.txt'
...
visual = Visiualization(wordlist)
visual.makeWorCloud()
visual.makeBarChart()
