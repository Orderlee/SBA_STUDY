# 회귀분석 : 광고 지출비에 따른 광고 수익 (단순 회귀 분석)

'''
회귀 분석 :  종속 변수 1개와 한 개 이상의 독립변수간의 인과 관계를 선형적으로 분석하는 기법
독립 변수의 개수에 따라
    단순 회귀 분석 : H = wx +b
...
머신러닝에서 w를 가중치(기울기)라고 하고, b를 bias라고 부름
회귀계수 : w와 b
비용함수(cost function): 회귀분석의 성능 지표
    총누적((예측값-실제값)**2)
    가장 시아적인 데이터는 0에 가깝다.

최소 자승법
    비용 함수를 구할 때 제곱을 해서 풀어나가므로 이를 최소 제곱법 또는 최소 자승법이라고 함

'''

'''
광고 지출비와 광고 수익에 따른 산점도/선 그래프
해당 데이터들이 선형적이라고 가정하고, 가장 시상적인 직선의 방정식을 구하도록 함
'''
...
x = np.array([1,2,3,4]) # 광고 지출비

y = np.array([30,70,90,115]) #광고 수익
...
# 실제 데이터 (정답_label_) 와 예측 데이터의 차이를 수직선으료 표현
...
plt.plot(x,best, marker='o', color='y', linestyle='solid',label=mylabel)
...
# w 값이 27.5보다 작아지거나 커지면 비용 함수의 값은 커짐
# 결과를 토대로 2차 함수가 그려지는지 확인해 보자

# w를 20이상 35이하의 범위에서 w에 따른 비용함수를 시각화
...
# 결론 : x축에 w를 그리고, y축에 비용함수의 결과 그래프를 확인해보면
# 2차함수라는 것을 확인 할 수 있음
