...
'''
콜백함수: 시스템이 어떠한 요구 사항을 충족 했을 때, 스스로 동작하는 함수
ModelCheckPoint :  매 epochs마다 모델을 자동으로 저장해주는 콜백 함수
EarlyStopping : 학습이 진행되는 동안 개선의 여지가 보이지 않으면 강제로 스톱해주는 콜백함
'''
'''
파일을 읽어, 전체 15%만 샘플링
모델을 생성한 다음 학습 진행
매 epochs마다 모델을 학습시켜 업데이트 한 후 해당 모델을 저장
콜백함수에 대한 객체를 생성
fit() 함수의 callbacks 매개 변수에 콜백 객체를 대입해줍니다.
History 객체를 이용하여 비용함수와 정확도 정보를 확인
비용함수와 정확도에 대한 시각화를 수행
'''

filename='../logisticexample/wine.csv'
...
model_name = model_dir + '{epoch:04d}-{val_loss:.4f}.hdf5'

from tensorflow.python.keras.callbacks import ModelCheckpoint
from tensorflow.python.keras.callbacks import EarlyStopping

# filepath : 파일을 저장할 경로, monitor : 모니터링할 대상
# loss(학습셋 오차) ,val_loss(검증용셋 오차) ,acc(학습 정확도), val_acc(검증요셋 정확도)
...
# validation_split : 훈련에 반영하지 않고, 테스트용으로 사용할 데이터셋의 비율 실제 학습에 사용되지 않고,
#                    매 epochs의 끝 자락에서 metrics를 평가하는 용도로 사용
# callbacks : 콜백함수 객체들을 리스트 형식으로 지정해 줌
